...
// All MeraDoc traffic goes through our same-origin reverse proxy
// (app/api/mera/[...path]). MeraDoc sends no CORS headers, so direct browser
// calls are blocked; routing through the proxy also keeps the tenant secrets
// (x-api-token / originToken) server-side instead of in the client bundle.
// This proxy also forwards X-Idempotency-Key (used by appointment booking).
const PROXY_BASE = "/api/mera";
...
async function refreshAccessToken() {
  // Same-origin proxy — a direct browser call to MeraDoc's /sso/tenant is
  // CORS-blocked (MeraDoc sends no Access-Control headers).
