...
function MedicineOrdersPanel() {
  ...
  const loadOrders = async (silent = false) => {
    ...
    try {
      const email = localStorage.getItem("userEmail") || "";
      const stored = JSON.parse(localStorage.getItem(`medOrders_${email}`) || "[]");
      if (!stored.length) { setOrders([]); return; }

      // Fetch live status for each stored order in parallel
      const token = localStorage.getItem("accessToken") || "";
      const results = await Promise.allSettled(
        stored.map((rec) =>
          fetch(`/api/medicine/order/status?orderId=${encodeURIComponent(rec.orderId)}`, {
            headers: token ? { Authorization: `Bearer ${token}` } : {},
          })
            .then((r) => r.json())
            .then((json) => ({ ...rec, live: json?.data || null }))
            .catch(() => ({ ...rec, live: null }))
        )
      );
      setOrders(results.map((r) => r.status === "fulfilled" ? r.value : null).filter(Boolean));
    } catch (e) {
